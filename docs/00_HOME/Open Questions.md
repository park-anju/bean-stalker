...
**Status:** RESOLVED for the raw-coordinate decision in H11; the broader human-friendly fallback remains a future UX item.

**Resolution:** H11 removed raw latitude/longitude entry from normal user UI and made current-location acquisition automatic. It did not add Google Places Autocomplete, geocoding or another provider. A future human-friendly place/address fallback may be designed separately; it must preserve the canonical `SearchCenter` boundary and receive an explicit cost/privacy/provider decision. Until then, denied users receive browser-settings guidance and explicit retry rather than a developer-oriented coordinate form.
