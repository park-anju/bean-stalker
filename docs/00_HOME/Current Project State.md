...
**Phase:** T00 (bootstrap), T01 (shared contracts + env validation), T02 (distance/filter/favourite domain helpers), T03 (responsive React shell/routes), T04 (current + manual location resolution), and T05 (Fastify cafe search + Google provider adapter) complete; no map, frontend search orchestration, filter/sort UI, or favourite persistence implemented yet.
...
- `pnpm lint`, `pnpm format`, `pnpm typecheck`, `pnpm test` (130 tests across 4 packages), `pnpm build`, `pnpm e2e` (10 tests), and `pnpm dev` (all four workspace dev processes, with a `predev` hook guaranteeing shared packages are built before the parallel watchers start) all run real tooling and pass;
- git repository initialized; brain baseline, T00 bootstrap, T01 contracts/env, T02 domain helpers, T03 shell/routes, T04 location resolution, and T05 cafe search are separate commits;
...
- actual Google Maps credentials or billing configuration (local `.env`/`.env.local` files use non-functional placeholders, gitignored — a live Google Places smoke test was not performed for exactly this reason);
- deployed app;
...
A review checkpoint is expected before starting the next task. T06 (Maps JavaScript integration) is READY. T07 (search orchestration) remains PENDING until T06 is also DONE (T04 and T05 are already DONE). See [[Task Status]].
