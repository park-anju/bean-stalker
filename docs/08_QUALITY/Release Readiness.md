...
### Behaviour
- [x] Automatic geolocation success/error/retry works; prompt stays pending until
  Web Geolocation resolves; Permissions API is optional; insecure context has
  truthful HTTPS guidance; raw coordinate UI is absent — H11 unit + Playwright
  evidence. Human-friendly manual fallback remains a documented limitation.
...
- [x] keyboard/accessibility smoke test — H08: keyboard-only core flow, visible focus, assertive location-error alerts, WCAG AA contrast, WCAG 2.2 checkbox target size; current `@axe-core/playwright` zero-violation scan of 10 states. Formal audit still out of scope.
...
- [x] [[Current Project State]] reflects reality — kept current through H11.
- [x] [[Traceability Matrix]] has H11 automatic-location evidence links/status.
- [x] [[Implementation Handoffs]] records final commands/results — through H11.
...
**Current:** NOT READY for public/live release — `T08` live-provider smoke and all Google-side + deployment configuration remain **BLOCKED** ([[Known Blockers|BLK-001]] / [[Known Blockers|BLK-003]] / [[Known Blockers|BLK-004]]). The fixture-backed engineering + documentation work (P0 feature set, H02–H11) is complete and verified.
