...
| H09 | Architecture documentation | H08 | READY | [[SDD]], [[System Architecture]] |
| H10 | Portfolio README preparation | H09 | PENDING | [[Release Readiness]], [[Golden Demo Scenario]] |

## Notes

- **`H09` is the next `READY` task** (dependency `H08` `DONE`). H08 delivered
  the mobile/accessibility QA pass on the fixture-backed app: darkened the
  accent token to clear WCAG AA contrast, enlarged the "Open now only"
  checkbox to the WCAG 2.2 target-size minimum, gave location errors an
  assertive `role="alert"` associated with the coordinate fields, added an
  `axe-core` scan of 9 representative states plus a 320 px / keyboard /
  long-content Playwright suite. See [[Implementation Handoffs]] `H08`.
- With `T09`/`T10` and H02–H08 `DONE`, **no `T`-numbered task is `READY`** — every remaining `T11`–`T15` sits behind `T08`.
...
- Ordinary MVP feature implementation is complete after T10 ([[MVP Scope]] P0 feature set); H02–H08 (logging privacy, cost controls, graceful exhaustion, secrets/config hardening, API security, mobile/accessibility QA) are complete. Remaining pre-release work: H09–H10, then the T08-gated live smoke / failure-race hardening / verification / deploy / demo.
